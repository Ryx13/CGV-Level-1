...
loader.load('assets/zombie_corpse.glb', (gltf) => {
  const obj = gltf.scene;
  const box = new THREE.Box3().setFromObject(obj);
  const size = new THREE.Vector3(); box.getSize(size);
...
const ZOMBIE_TARGET_HEIGHT = 1.75;
...
loadZombieTemplate('assets/zombie_rigged.glb');
...
new GLTFLoader().load('assets/player_character.glb', (gltf) => {
  ...
  if (gltf.animations && gltf.animations.length) {
    ...
    if (idleClip) playerVis.actions.idle = playerVis.mixer.clipAction(idleClip); // a real standing/aiming stance now, not a paused walk frame
    if (jumpClip) {
    ...
    // Pistol_Shoot exists and loads, but is deliberately NOT auto-triggered
    // on fire this pass: at ~9 rounds/sec automatic fire, restarting a
    // full-body animation on every single shot would fight the walk/sprint
    // pose constantly and likely look worse than not having it at all.
    // Doing this properly needs upper-body-only blending (an arm/spine
    // bone mask) so the shoot pose can layer over the legs' walk cycle
    // without overriding it — that's a real feature, not a quick wire-up,
    // so it's not in this pass. The action is loaded and ready for when
    // that's built.
    if (shootClip) playerVis.shootAction = playerVis.mixer.clipAction(shootClip);
...
class Zombie {
  constructor(pos) {
    ...
    if (this.rigged) {
      ...
      this.footBoneL = rigInstance.getObjectByName('L_Ankle') || null;
      this.footBoneR = rigInstance.getObjectByName('R_Ankle') || null;
    ...
    this.speed = 1.0 + Math.random() * 1.0; // was 1.35-2.9, toned down to a shamble-to-jog range
  ...
  takeDamage(dmg, headshot) {
    ...
    // The story's core combat twist: a shot never permanently kills on its
    // own. It "stabilizes" them — they drop and look dead — but they get
    // back up later unless you finish them while they're down. This is
    // handled here rather than in tryShoot/tryMelee so both weapons behave
    // consistently: a hit on an already-downed zombie is a finishing hit.
    if (this.state === 'downed') { this.confirmedKill(headshot); return; }
    this.health -= dmg;
  ...
  goDown() {
    ...
    // Kills now count right here, on knockdown — not on a separate
    // "finishing hit" the player was never told about. That gate was the
    // direct cause of the objective counter appearing stuck: you'd drop
    // zombies all day and the tally would never move because nothing
    // told you a follow-up hit was required. The guard below stops the
    // same zombie being counted twice if it reanimates and gets dropped
    // again later.
    if (!this.countedKill) {
  ...
  confirmedKill(headshot) {
    // No longer touches `kills` (see goDown) — this just makes the drop
    // permanent instead of a temporary knockdown, and drops loot.
    if (!this.alive) return;
    this.alive = false;
    this.state = 'dead';
    this.deathTimer = 6;
  ...
  update(dt, playerPos) {
    ...
    if (this.state === 'chase') {
      ...
      // Animation playback rate now tracks actual movement speed more
      // proportionally (was 1.35 + speed*0.15 ≈ 1.55-1.8x regardless of
      // how slow the zombie actually was) — the mismatch between fast
      // limb-cycling and modest ground speed likely read as "moving really
      // fast" even before the speed values themselves were reduced above.
      this.playWalk(dt, 8, 0.85 + this.speed * 0.25);
...
function activeGunMesh() {
  return firstPerson ? viewmodelGun : playerVis.gun;
}
...
function startReload() {
  if (reloading || ammoMag === MAG_SIZE || ammoReserve <= 0 || inVehicle) return;
...
function updateObjectiveHUD() {
  ...
  if (stage === 1) {
    dom.objective.innerHTML = `Objective: <b>Fight through to the depot</b> — neutralize <span id="obj-count">${Math.min(kills, KILL_TARGET)}/${KILL_TARGET}</span> infected. <i>(Look east for an amber glow above the rooftops — that's the depot, off the main road. A supply cache is hidden west too.)</i>`;
  } else if (stage === 2) {
...
function damagePlayer(dmg) {
  ...
  dom.healthFill.style.width = playerHealth + '%';
  if (dom.bloodVig) dom.bloodVig.style.opacity = String(1 - playerHealth / 100);
  sfx.hurt();
...
function doDeath() {
  ...
  dom.deathStats.textContent = `Infected eliminated: ${kills}`;
...
function doWin() {
  ...
  sfx.setEngine(0, false);
  document.exitPointerLock();
  dom.winStats.textContent = `Infected eliminated: ${kills} · Extraction successful.`;
...
function updatePlayerMovement(dt) {
  ...
  if (dom.debugReadout) {
    ...
    dom.debugReadout.textContent =
      ...
      `Grip preset: ${gunGripIndex} (U to cycle)`;
  }
...
function updateVehicleControls(dt) {
  ...
  const targetThrottle = throttleInput > 0 ? 1 : (throttleInput < 0 ? -0.55 : 0);
  ...
  for (let i = 0; i < 4; i++) {
    let b = 0;
    if (throttleInput === 0) b = MAX_BRAKE_FORCE * 0.4;
...
function animate() {
  ...
  if (!isDead && dom.hud && !dom.hud.classList.contains('hidden')) {
    ...
    if (firePressed && !inVehicle) tryShoot();
    ...
    if (inVehicle) {
      ...
      viewmodelGun.visible = firstPerson;
